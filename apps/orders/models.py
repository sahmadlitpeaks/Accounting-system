...
class CustomerCreditNote(TimeStampedModel):
    """A credit note against a customer invoice (sales return / correction).

    In Pakistan this is the sanctioned way to correct a cleared invoice
    (the FBR 72-hour amendment rule); it is fiscalized like an invoice.
    Shares the fiscal-document shape so the same adapters apply.
    """

    document_kind = "credit_note"  # read by fiscalization adapters

    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="customer_credit_notes")
    party = models.ForeignKey(Party, on_delete=models.PROTECT, related_name="customer_credit_notes")
    invoice = models.ForeignKey(CustomerInvoice, on_delete=models.PROTECT, related_name="credit_notes")
    date = models.DateField()
    currency = models.ForeignKey(Currency, on_delete=models.PROTECT, related_name="+")
    fx_rate = models.DecimalField(max_digits=18, decimal_places=8, default=Decimal("1"))
    number = models.CharField(max_length=40, blank=True)
    ...
    fiscal_status = models.CharField(max_length=14, choices=FiscalStatus.choices, default=FiscalStatus.PENDING)
    fbr_invoice_number = models.CharField(max_length=64, blank=True)
    qr_payload = models.TextField(blank=True)
    peppol_id = models.CharField(max_length=128, blank=True)
    ...
    def __str__(self):
        return self.number or f"CRN-draft-{self.pk}"


class CustomerCreditNoteLine(TimeStampedModel):
    credit_note = models.ForeignKey(CustomerCreditNote, on_delete=models.CASCADE, related_name="lines")
    item = models.ForeignKey(Item, on_delete=models.PROTECT, related_name="+")
    description = models.CharField(max_length=255, blank=True)
    quantity = models.DecimalField(max_digits=18, decimal_places=4)
    unit_price = models.DecimalField(max_digits=18, decimal_places=4)
...
class SupplierBill(TimeStampedModel):
    ...
    net_total = models.DecimalField(max_digits=18, decimal_places=2, default=ZERO)
    tax_total = models.DecimalField(max_digits=18, decimal_places=2, default=ZERO)
    # Withholding tax deducted at source (Pakistan income-tax WHT). Reduces the
    # amount payable to the supplier and creates a liability to the tax authority.
    withholding_tax_code = models.ForeignKey(TaxCode, null=True, blank=True, on_delete=models.PROTECT, related_name="+")
    withholding_total = models.DecimalField(max_digits=18, decimal_places=2, default=ZERO)
    grand_total = models.DecimalField(max_digits=18, decimal_places=2, default=ZERO)
    amount_paid = models.DecimalField(max_digits=18, decimal_places=2, default=ZERO)
    status = models.CharField(max_length=12, choices=DocumentStatus.choices, default=DocumentStatus.DRAFT)
    journal_entry = models.ForeignKey("accounting.JournalEntry", null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    ...
    @property
    def amount_due(self) -> Decimal:
        # We owe the supplier the gross less any tax withheld at source.
        return self.grand_total - self.withholding_total - self.amount_paid
